...
export class FavoriteService {
  private readonly favoritesKeyPrefix = 'ecommerce_favorites_';
  ...
  private getFavoritesKey(): string {
    const userId = this.authState.currentUser()?.customerId;
    if (!userId) {
      return this.favoritesKeyPrefix + 'guest';
    }
  ...
  private loadFavorites(): ProductResponseDTO[] {
    try {
      const raw = globalThis.localStorage?.getItem(this.getFavoritesKey());
  ...
  private persist(next: ProductResponseDTO[]): void {
    try {
      globalThis.localStorage?.setItem(this.getFavoritesKey(), JSON.stringify(next));
    } catch {
