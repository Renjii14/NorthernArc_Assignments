...
export class CartService {
  private readonly cartKeyPrefix = 'ecommerce_cart_';
  ...
  private getCartKey(): string {
    ...
    if (!userId) {
      return this.cartKeyPrefix + 'guest';
    }
  ...
  private loadCart(): CartItem[] {
    try {
      const raw = globalThis.localStorage?.getItem(this.getCartKey());
  ...
  private persist(items: CartItem[]): void {
    try {
      globalThis.localStorage?.setItem(this.getCartKey(), JSON.stringify(items));
    } catch {
