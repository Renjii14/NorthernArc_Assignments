...
export class LoanProductsComponent implements OnInit {
    ...
    this.loanService.applyForLoan(request).subscribe({
      next: (response) => {
        ...
        this.status.set({
          loading: false,
          error: '',
          success: `Application submitted for ${product.displayName}. Application ID: ${response.applicationId}. Track status in My Requests.`
        });
    ...
    this.loanService.applyForLoan({
      ...
      next: (response) => {
        ...
        this.status.set({
          loading: false,
          error: '',
          success: `Re-application submitted successfully. New Application ID: ${response.applicationId}.`
        });
  ...
  canCreateLoanFromApplication(application: LoanApplicationDTO): boolean {
    return this.normalizeApplicationStatus(application.applicationStatus) === 'APPROVED';
  }

  approveApplication(application: LoanApplicationDTO): void {
    this.processApplicationDecision(application, 'APPROVED');
  }
  ...
  rejectApplication(application: LoanApplicationDTO): void {
    ...
    if (!rejectionReason) {
      this.toastService.show('Rejection reason is required', 'warning');
      return;
    }

    this.processApplicationDecision(application, 'REJECTED', rejectionReason);
  }
  ...
  private processApplicationDecision(application: LoanApplicationDTO, status: 'APPROVED' | 'REJECTED', rejectionReason?: string): void {
    this.reviewQueueState.set({
    ...
    this.loanService.updateLoanApplicationStatus(application.applicationId, { status, rejectionReason }).subscribe({
      next: () => {
        ...
        this.reviewQueueState.set({
          loading: false,
          error: '',
          success: `Application #${application.applicationId} ${status.toLowerCase()}.`,
          processingId: 0
        });
        ...
        if (approvedNow) {
          const navigateToCreateLoan = window.confirm(
            `Application #${application.applicationId} approved. Do you want to create the loan now?`
          );
          if (navigateToCreateLoan) {
