      ...
      .pipe(
        catchError((error) => {
          if (error?.status === 404 || error?.status === 405) {
            return this.http.post<RegisterResponse>(`${this.baseUrl}/customers/register`, payload);
          }
          return throwError(() => error);
        }),
        catchError((error) => {
          if (error?.status === 404 || error?.status === 405) {
            return this.http.post<RegisterResponse>(`${this.baseUrl}/customers`, payload);
          }
          return throwError(() => error);
        }),
        catchError(handleHttpError('Register request'))
      );
  }
