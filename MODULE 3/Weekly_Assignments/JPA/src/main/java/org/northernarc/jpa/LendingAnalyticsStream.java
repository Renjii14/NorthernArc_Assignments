...
public class LendingAnalyticsStream {
...
//    Higher Credit Score
//    Lower Loan Amount
//    Smaller Customer Name
//
    public void loadApplications(List<String> records) {
        if(records == null) {
            return;
        }
        records.stream().filter(Objects::nonNull).map(String::trim)
                .filter(s -> !s.isEmpty())
                .forEach(record -> {
                    ...
                    LoanApplication current = new LoanApplication(applicationId, customerName, lenderName, loanType, loanAmount, creditScore);
                    applications.merge(
                            applicationId,
                            current,
                            (oldApp,newApp) -> {

                                if(newApp.getCreditScore()
                                        > oldApp.getCreditScore()) {
                                    return newApp;
                                }
                                ...
                                if(newApp.getLoanAmount()
                                        < oldApp.getLoanAmount()) {
                                    return newApp;
                                }
...
//    Loan Amount ASC
    ...
    public List<String> suspiciousApplications() {
        ...
        Map<String, Double> avgAmountByType =
                apps.stream().collect(Collectors.groupingBy(
                        LoanApplication::getLoanType,
                                Collectors.averagingDouble(
                                        LoanApplication::getLoanAmount)));
...
//        Condition 6 Rule
//        Same customer has applied through more than 3 lenders.
...
//                    Condition 2
//
//                    Lender inside customer name
                    boolean condition2 =
                            customer.toLowerCase()
                                    .contains(
                                            lender.toLowerCase());
//                    Condition 3
//                    Loan amount > 250% average
                    boolean condition3 =
                            app.getLoanAmount()
                                    >
